...
The **Código Interface Description Language (CIDL)** is a formal language used to describe the interface of a blockchain smart contract. It enables the expression and creation of programs of any level of complexity—from a single contract to multi‑contract applications—discoverable and callable by other contracts or users. By standardizing interfaces and data structures, CIDL **accelerates development** and **reduces security risks** by eliminating boilerplate and ambiguity.

The CIDL is written in **YAML**. The preferred file extension is **`.cidl`**, though **`.yaml`** and **`.yml`** are also supported.

CIDL also takes inspiration from the **Solana IDL file**, but it extends beyond it to describe multi‑chain and advanced features.

---

## Quickstart

Every CIDL begins by declaring the specification version:
...
Check the **What’s New / Changelog** to find the latest supported spec version.

---

## Why CIDL (Goals)

CIDL is designed to speed blockchain adoption by letting developers focus on **business logic** while generators handle the rest:

1. **Smart Contract stubs & boilerplate**

   * Auto‑generated, validated, and audited stubs abstract initialization, verification, and parsing. You implement only the logic.
2. **Client library generation**

   * From one CIDL, generate client code for multiple languages with simple APIs that wrap RPC details.
3. **Documentation generation**

   * Produce full, up‑to‑date contract docs from the CIDL: exposed methods, inputs, and (for account‑based VMs like Solana) account structures.
...
To dive deeper into CIDL, explore each section:
...
Código is a growing community of developers. Join us on **[Discord](https://discord.gg/8XHQGS832k)** and **[GitHub](https://github.com/Codigo-io)**.
